...
function onUpdate(self, elapsed)
    ...
    local GetNumRaidMembers, UnitIsUnit, UnitIsDeadOrGhost = GetNumRaidMembers, UnitIsUnit, UnitIsDeadOrGhost
    local UnitClass, GetRaidTargetIndex, UnitName = UnitClass, GetRaidTargetIndex, UnitName
    ...
    for i = 1, GetNumRaidMembers() do
        ...
        if not UnitIsUnit(uId, "player") and not UnitIsDeadOrGhost(uId) and self.checkFunc(uId) then
            j = j + 1
            local color = RAID_CLASS_COLORS[select(2, UnitClass(uId))] or NORMAL_FONT_COLOR
            local icon = GetRaidTargetIndex(uId)
            local text = icon and format("|TInterface\\TargetingFrame\\UI-RaidTargetingIcon_%d:0|t %s", icon, UnitName(uId)) or UnitName(uId)
            self:AddLine(text, color.r, color.g, color.b)
            if j >= 7 then break end  
        end
    end

    local titleLine = _G[self:GetName().."TextLeft1"]
    if titleLine then
        if j >= 7 then
            titleLine:SetText("МЕГАПАКЕТ!")
            titleLine:SetTextColor(1, 0, 0)  
        else
            titleLine:SetText("Кто в пакете")
            titleLine:SetTextColor(1, 1, 1)  
        end
    end
